...
std::vector<std::optional<Eigen::Vector3d>> triangulate_triple_match(
  ...
  std::vector<CameraParams*> intrinsic_calibrations = {
      &calib->left_intrinsics, &calib->color_intrinsics,
  ...
  for (int i = 0; i < intrinsic_calibrations.size(); i++) {
    ...
    cv::undistortPoints(pixels, cv_normalized_points, eigen_to_cv_mat(calib->K),
  ...
  std::vector<Eigen::Matrix4d> cam_positions = {calib->center_to_left,
  ...
  for (int point_idx = 0; point_idx < triangulated.size(); point_idx++) {
    ...
    for (int camera_idx = 0; camera_idx < intrinsic_calibrations.size();
      ...
      cv::projectPoints(
          cv::Mat(eigen_vec3_to_cv(point_in_world)), eigen_to_cv_mat(rvec),
      ...
      cv::Point2d cv_out_point(cv_out_mat.at<double>(0, 0),
                               cv_out_mat.at<double>(0, 1));

      double repr_error = cv::norm(cv::Point2d(measured) - cv_out_point);
      if (repr_error > repr_error_threshold) {
        spdlog::debug(std::format("bad reprojection error: {} camera {}",
                                  repr_error, camera_idx));
        bad = true;
        break;
      }
    }
...
std::vector<std::optional<Eigen::Vector3d>> triangulate_normalized_image_points(
  ...
  for (int point_idx = 0; point_idx < num_points; point_idx++) {
    ...
    for (int cam_idx = 0; cam_idx < num_cameras; cam_idx++) {
      Eigen::Vector2d xy = normalized_image_points->at(cam_idx)[point_idx];
      Eigen::Vector3d d = Eigen::Vector3d(xy.x(), xy.y(), 1.0).normalized();
    ...
    for (int cam_idx = 0; cam_idx < num_cameras; cam_idx++) {
      ...
      Eigen::Vector3d proj = dir * (diff.dot(dir));  // Projection onto the ray
