...
OrbDetection::OrbDetection(std::vector<cv::KeyPoint> keypoints,
                           cv::Mat descriptors)
    : keypoints(keypoints), descriptors(descriptors) {}

// Move constructor
OrbDetection::OrbDetection(OrbDetection&& other)
...
OrbDetection OrbDetector::detect(cv::Mat img) {
...
OrbMatcher::OrbMatcher() : matcher(OrbMatcher::make_matcher()) {}
...
std::vector<cv::DMatch> OrbMatcher::good_matches(cv::Mat descriptors1,
                                                 cv::Mat descriptors2,
                                                 int min_num_matches) {
  std::vector<cv::DMatch> matches;
  spdlog::debug(std::format("Making good matches from {} and {} descriptors",
...
TripleMatch OrbMatcher::make_triple_match(OrbDetection& center_orb,
  ...
  for (int cl_idx = 0; cl_idx < center_to_left_matches.size(); cl_idx++) {
    ...
    for (int cr_idx = 0; cr_idx < center_to_right_matches.size(); cr_idx++) {
      ...
      if (cr_match.queryIdx == center_idx) {
        ...
        center_descriptors.push_back(center_orb.descriptors.row(center_idx));
