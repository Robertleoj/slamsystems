...
struct PoseGraphVertex {
    ...
    PoseGraphVertex(
        int id,
        py::EigenDRef<Eigen::Matrix4d> pose
    )
        : id(id),
          pose(pose) {}
};
...
struct PoseGraphEdge {
    ...
    PoseGraphEdge(
        int v1_id,
        int v2_id,
        py::EigenDRef<Eigen::Matrix4d> v1_to_v2
    )
        : v1_id(v1_id),
          v2_id(v2_id),
          v1_to_v2(v1_to_v2) {}
...
std::vector<Eigen::Matrix4d> pose_graph_ba(
    ...
    for (auto& e : edges) {
        factors.push_back(sym::Factord::Hessian(
            circle_pose_graph::PoseDiffFactor<double>,
            {{Var::POSE, e.v1_id},
             {Var::POSE, e.v2_id},
    ...
    for (auto& e : edges) {
        values.Set(
            {Var::POSE_DIFF, e.v1_id, e.v2_id}, pose_from_homo_mat(e.v1_to_v2)
        );
    }
    ...
    sym::Optimizerd optimizer(
        optimizer_params, factors, "PoseGraphOptimizer", {}, epsilon
    );
    ...
    for (int i = 0; i < num_vertices; i++) {
        out.push_back(
            values.At<sym::Pose3d>({Var::POSE, i}).ToHomogenousMatrix()
        );
    }
