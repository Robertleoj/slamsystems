...
struct FeatureMatchFrameData {
  ...
  FeatureMatchFrameData(FeatureMatchFrameData&& other) noexcept
...
class OrbMatcher {
 public:
  OrbMatcher();
  std::vector<cv::DMatch> good_matches(cv::Mat des1,
                                       cv::Mat des2,
                                       int min_num_matches = 10);

  TripleMatch make_triple_match(OrbDetection& center_orb,
