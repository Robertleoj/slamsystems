...
struct CameraPose {
    ...
    CameraPose(
        int frame_id,
        py::EigenDRef<Eigen::Matrix4d> camera_pose
    )
        : frame_id(frame_id),
...
struct Landmark {
    ...
    Landmark(
        int id,
        py::EigenDRef<Eigen::Vector3d> loc
    )
        : id(id),
          loc(loc) {}
};
...
std::tuple<std::vector<CameraPose>, std::vector<Landmark>> depth_slam_ba(
    ...
    for (auto& c : camera_poses) {
        values.Set(
            {Var::CAM_POSE, c.frame_id}, pose_from_homo_mat(c.camera_pose)
        );
    }
    ...
    sym::Optimizerd optimizer(
        optimizer_params, factors, "DepthSlamOptimizer", {}, epsilon
    );
    ...
    for (auto& c : camera_poses) {
        Eigen::Matrix4d pose =
            values.At<sym::Pose3d>({Var::CAM_POSE, c.frame_id})
                .ToHomogenousMatrix();
