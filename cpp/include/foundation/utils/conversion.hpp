...
cv::Mat eigen_to_cv_mat(
    Eigen::Matrix<T, R, C> eig
) {
    cv::Mat m;
    cv::eigen2cv(eig, m);
    return m;
...
template <typename T>
cv::Point_<T> eigen_vec2_to_cv(
    Eigen::Vector2<T> eigen_vec
) {
...
template <typename T>
Eigen::Vector2<T> cv_vec2_to_eigen(
    cv::Point_<T> cv_vec
) {
...
template <typename T>
cv::Point3_<T> eigen_vec3_to_cv(
    Eigen::Vector3<T> eigen_vec
) {
...
template <typename T>
Eigen::Vector3<T> cv_vec3_to_eigen(
    cv::Point3_<T> cv_vec
) {
