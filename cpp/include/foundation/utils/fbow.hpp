...
cv::Mat desc_array_to_cv(py::array_t<float, py::array::c_style> &desc) {
...
void vocab_save(DBoW3::Vocabulary &vocab, std::filesystem::path &path) {
  vocab.save(path.string());
}
void vocab_load(DBoW3::Vocabulary &vocab, std::filesystem::path &path) {
...
void db_add(DBoW3::Database &db, py::array_t<float, py::array::c_style> &desc) {
...
std::vector<DBoW3::Result> db_query(
    DBoW3::Database &db, py::array_t<float, py::array::c_style> &desc,
    int max_results) {
  auto desc_cv = desc_array_to_cv(desc);
